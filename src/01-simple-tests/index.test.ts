...
describe('simpleCalculator tests', () => {
  ...
  test('should exponentiate two numbers', () => {
    expect(simpleCalculator({ a: 6, b: 3, action: Action.Exponentiate })).toBe(
      216,
    );
  });

  test('should return null for invalid action', () => {
    expect(simpleCalculator({ a: 6, b: 3, action: 'InvalidAction' })).toBe(
      null,
    );
  });
  ...
  test('should return null for invalid arguments', () => {
    expect(
      simpleCalculator({ a: 'asd', b: 'bsd', action: 'InvalidAction' }),
    ).toBe(null);
  });
});
